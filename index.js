...
function generateRandomPassword(length) {
 ...
 let string = "";
  ...
  for (let i = 0; i < length; i++) {
    ...
    let randSymbol = characters[index];
    string += randSymbol;
  }
  return string;
}
...
function calculateHypotenuse(a, b) {
  ...
  if (typeof a === "number" && typeof b === "number") {
    const gipotenuza = Math.sqrt(Math.pow(a, 2) + Math.pow(b, 2));
    return `довжина гіпотенузи для прямокутного трикутника з катетами довжиною ${a} та ${b}, буде дорівнювати ${gipotenuza}.`;
  } else {
    return null;
  }
}
...
function getFractionalPart(num) {
  ...
  const last = num - Math.floor(num);
  return Math.fround(last);
}
...
function estimateInvestment(principal, interestRate, years) {
  ...
  if (
    ...
    console.log("Hе всі аргументи є числами");
    return null;
  }
  //console.log(principal, 1 + interestRate, years);
...
function isTotalPriceExceedsMaxPrice(products, maxPrice) {
  ...
  if (Array.isArray(products)) {
    if (typeof maxPrice === "number") {
      let totalPrice = products.reduce((accum, elem) => accum + elem.price, 0);
